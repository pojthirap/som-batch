...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOfficeToSaleRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOfficeToSaleResponse;
...
{
    public  class SaleOfficeToSaleFunction
    {
        ...
        public SaleOfficeToSaleFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOfficeToSaleFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOfficeToSaleFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI007', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOfficeToSaleRequest reqData = new OutboundSaleOfficeToSaleRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI007";
                input.Table_Object = "V_TVKBZ_ASSIGN";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale);
                ...
                OutboundSaleOfficeToSaleResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOfficeToSaleResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOfficeToSaleResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficeToSale);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" FROM ORG_SALE_OFFICE_AREA OA ");
                                    bulder.AppendFormat(" INNER JOIN ORG_SALE_AREA SA ON SA.AREA_ID = OA.AREA_ID ");
                                    bulder.AppendFormat(" where SA.ORG_CODE = @Sales_Org and SA.CHANNEL_CODE = @Distribution_Channel and SA.DIVISION_CODE = @Division and OA.OFFICE_CODE = @Sales_Office ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        bulder.AppendFormat(" INSERT INTO ORG_SALE_OFFICE_AREA ([OFFICE_AREA_ID], [OFFICE_CODE], [AREA_ID], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" SELECT NEXT VALUE FOR ORG_SALE_OFFICE_AREA_SEQ, @Sales_Office, SA.AREA_ID, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME() FROM ORG_SALE_AREA SA where SA.ORG_CODE = @Sales_Org and SA.CHANNEL_CODE = @Distribution_Channel and SA.DIVISION_CODE = @Division ");
                                        command.Parameters.Add("@Sales_Office", SqlDbType.NVarChar);
                                        command.Parameters["@Sales_Office"].Value = data.Sales_Office;
                                        command.Parameters.Add("@Sales_Org", SqlDbType.NVarChar);
                                        command.Parameters["@Sales_Org"].Value = data.Sales_Org;
                                ...
                                bulder.AppendFormat(" UPDATE ORG_SALE_OFFICE_AREA   ");
