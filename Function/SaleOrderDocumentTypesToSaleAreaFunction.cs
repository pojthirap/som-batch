...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOrderDocumentTypesToSaleAreaRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOrderDocumentTypesToSaleAreaResponse;
...
{
    public class SaleOrderDocumentTypesToSaleAreaFunction
    {
        ...
        public SaleOrderDocumentTypesToSaleAreaFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOrderDocumentTypesToSaleAreaFunction")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOrderDocumentTypesToSaleAreaFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI017', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOrderDocumentTypesToSaleAreaRequest reqData = new OutboundSaleOrderDocumentTypesToSaleAreaRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI017";
                input.Table_Object = "V_TVAKZ";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea);
                ...
                OutboundSaleOrderDocumentTypesToSaleAreaResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOrderDocumentTypesToSaleAreaResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOrderDocumentTypesToSaleAreaResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypesToSaleArea);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE OA SET OA.ACTIVE_FLAG='Y',OA.SYNC_ID= @VAL_SYNC_DATA_LOG_SEQ ");
                                    bulder.AppendFormat(" FROM MS_ORDER_DOC_TYPE_AREA OA ");
                                    bulder.AppendFormat(" INNER JOIN ORG_SALE_AREA SA ON SA.AREA_ID = OA.AREA_ID ");
                                    bulder.AppendFormat(" where SA.ORG_CODE = @Sales_Org and SA.CHANNEL_CODE = @Distribution_Channel and SA.DIVISION_CODE = @Division and OA.DOC_TYPE_CODE = @Sale_Doc_Type ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Sales_Org", SqlDbType.NVarChar);
                                    command.Parameters["@Sales_Org"].Value = data.Sale_Org;
                                    ...
                                    command.Parameters.Add("@Sale_Doc_Type", SqlDbType.NVarChar);
                                    command.Parameters["@Sale_Doc_Type"].Value = data.Sale_Doc_Type;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_ORDER_DOC_TYPE_AREA ([DOC_TYPE_AREA_ID], [DOC_TYPE_CODE], [AREA_ID], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" SELECT NEXT VALUE FOR MS_ORDER_DOC_TYPE_AREA_SEQ, @Sale_Doc_Type, SA.AREA_ID, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME() FROM ORG_SALE_AREA SA where SA.ORG_CODE = @Sales_Org and SA.CHANNEL_CODE = @Distribution_Channel and SA.DIVISION_CODE = @Division  ");
                                        command.Parameters.Add("@Sale_Doc_Type", SqlDbType.NVarChar);
                                        command.Parameters["@Sale_Doc_Type"].Value = data.Sale_Doc_Type;
                                        command.Parameters.Add("@Sales_Org", SqlDbType.NVarChar);
                                        command.Parameters["@Sales_Org"].Value = data.Sale_Org;
                                        ...
                                        command.Parameters["@Sales_Office"].Value = data.Sales_Office;
                                        command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                        command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                        QueryUtils.configParameter(command);
                                        // Execute SQL
