...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleGroupInformationRequest;
using AzureFunctionApp.Model.Response.OutboundSaleGroupInformationResponse;
...
{
    public  class SaleGroupInformationFunction
    {
        ...
        public SaleGroupInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleGroupInformationFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleGroupInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI004', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleGroupInformationRequest reqData = new OutboundSaleGroupInformationRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI004";
                input.Table_Object = "V_TVKGR";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation);
                ...
                OutboundSaleGroupInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleGroupInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleGroupInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleGroupInformation);
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        bulder.AppendFormat(" INSERT INTO ORG_SALE_GROUP ([GROUP_CODE], [DESCRIPTION_TH], [DESCRIPTION_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Sales_Group, @Description, @Description, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Sales_Group", SqlDbType.NVarChar);
                                        command.Parameters["@Sales_Group"].Value = data.Sales_Group;
                                        command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                        command.Parameters["@Description"].Value = data.Description;
