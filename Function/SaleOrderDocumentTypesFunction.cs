...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOrderDocumentTypesRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOrderDocumentTypesResponse;
...
{
    public class SaleOrderDocumentTypesFunction
    {
        ...
        public SaleOrderDocumentTypesFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOrderDocumentTypesFunction")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOrderDocumentTypesFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI016', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOrderDocumentTypesRequest reqData = new OutboundSaleOrderDocumentTypesRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI016";
                input.Table_Object = "V_TVAK";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes);
                ...
                OutboundSaleOrderDocumentTypesResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOrderDocumentTypesResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOrderDocumentTypesResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderDocumentTypes);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_ORDER_DOC_TYPE  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [DOC_TYPE_NAME_TH]=@Desc, [DOC_TYPE_NAME_EN]=@Desc, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [DOC_TYPE_CODE]=@Sales_Doc_Type ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Desc", SqlDbType.NVarChar);
                                    command.Parameters["@Desc"].Value = data.Desc;
                                    command.Parameters.Add("@Sales_Doc_Type", SqlDbType.NVarChar);
                                    command.Parameters["@Sales_Doc_Type"].Value = data.Sales_Doc_Type;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_ORDER_DOC_TYPE ([DOC_TYPE_CODE], [DOC_TYPE_NAME_TH], [DOC_TYPE_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Sales_Doc_Type, @Desc, @Desc, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Sales_Doc_Type", SqlDbType.NVarChar);
                                        command.Parameters["@Sales_Doc_Type"].Value = data.Sales_Doc_Type;
                                        command.Parameters.Add("@Desc", SqlDbType.NVarChar);
                                        command.Parameters["@Desc"].Value = data.Desc;
