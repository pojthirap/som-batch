...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOrderItemTypeRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOrderItemTypeResponse;
...
{
    public class SaleOrderItemTypeFunction
    {
        ...
        public SaleOrderItemTypeFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOrderItemTypeFunction")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOrderItemTypeFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI019', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOrderItemTypeRequest reqData = new OutboundSaleOrderItemTypeRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI019";
                input.Table_Object = "V_TVAP";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType);
                ...
                OutboundSaleOrderItemTypeResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOrderItemTypeResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOrderItemTypeResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderItemType);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_ORDER_ITEM_TYPE  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [ITEM_TYPE_NAME_TH]=@Description, [ITEM_TYPE_NAME_EN]=@Description, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [ITEM_TYPE_CODE]=@Item_Category ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                    command.Parameters["@Description"].Value = data.Description;
                                    command.Parameters.Add("@Item_Category", SqlDbType.NVarChar);
                                    command.Parameters["@Item_Category"].Value = data.Item_Category;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_ORDER_ITEM_TYPE ([ITEM_TYPE_CODE], [ITEM_TYPE_NAME_TH], [ITEM_TYPE_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Item_Category, @Description, @Description, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Item_Category", SqlDbType.NVarChar);
                                        command.Parameters["@Item_Category"].Value = data.Item_Category;
                                        command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                        command.Parameters["@Description"].Value = data.Description;
