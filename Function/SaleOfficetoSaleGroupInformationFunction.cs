...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOfficetoSaleGroupInformationRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOfficetoSaleGroupInformationResponse;
...
{
    public  class SaleOfficetoSaleGroupInformationFunction
    {
        ...
        public SaleOfficetoSaleGroupInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOfficetoSaleGroupInformationFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOfficetoSaleGroupInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI005', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOfficetoSaleGroupInformationRequest reqData = new OutboundSaleOfficetoSaleGroupInformationRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI005";
                input.Table_Object = "V_TVBVK_ASSIGN";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation);
                ...
                OutboundSaleOfficetoSaleGroupInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOfficetoSaleGroupInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOfficetoSaleGroupInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOfficetoSaleGroupInformation);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE ORG_SALE_GROUP  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [OFFICE_CODE]=@Sales_Office, [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [GROUP_CODE]=@Sales_Group ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    ...
                                    command.Parameters["@Sales_Group"].Value = data.Sales_Group;
                                    QueryUtils.configParameter(command);
                                ...
                                bulder.AppendFormat(" UPDATE ORG_SALE_GROUP  ");
                                bulder.AppendFormat(" SET [OFFICE_CODE] = null, [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                bulder.AppendFormat(" WHERE ISNULL(SYNC_ID,-1) != @VAL_SYNC_DATA_LOG_SEQ AND CREATE_USER = 'SYSTEM'");
                                command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                // Execute SQL
                                logger.LogDebug("Query:" + bulder.ToString());
