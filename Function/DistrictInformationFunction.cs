...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundDistrictInformationRequest;
using AzureFunctionApp.Model.Response.OutboundDistrictInformationResponse;
...
{
    public  class DistrictInformationFunction
    {
        ...
        public DistrictInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("DistrictInformationFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. DistrictInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI030', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundDistrictInformationRequest reqData = new OutboundDistrictInformationRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI030";
                input.Table_Object = "V_TVV5";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation);
                ...
                OutboundDistrictInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundDistrictInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundDistrictInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundDistrictInformation);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_DISTRICT  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [DISTRICT_NAME_TH]=@Description, [DISTRICT_NAME_EN]=@Description, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [DISTRICT_CODE]=@Customer_group_5 ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                    command.Parameters["@Description"].Value = data.Description;
                                    command.Parameters.Add("@Customer_group_5", SqlDbType.NVarChar);
                                    command.Parameters["@Customer_group_5"].Value = data.Customer_group_5;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_DISTRICT ([DISTRICT_CODE], [DISTRICT_NAME_TH], [DISTRICT_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Customer_group_5, @Description, @Description, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Customer_group_5", SqlDbType.NVarChar);
                                        command.Parameters["@Customer_group_5"].Value = data.Customer_group_5;
                                        command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                        command.Parameters["@Description"].Value = data.Description;
