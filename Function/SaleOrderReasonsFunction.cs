...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundSaleOrderReasonsRequest;
using AzureFunctionApp.Model.Response.OutboundSaleOrderReasonsResponse;
...
{
    public class SaleOrderReasonsFunction
    {
        ...
        public SaleOrderReasonsFunction(IConfiguration configuration)
        {
        ...
        [Function("SaleOrderReasonsFunction")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. SaleOrderReasonsFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI018', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundSaleOrderReasonsRequest reqData = new OutboundSaleOrderReasonsRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI018";
                input.Table_Object = "V_TVAU";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons);
                ...
                OutboundSaleOrderReasonsResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundSaleOrderReasonsResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundSaleOrderReasonsResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundSaleOrderReasons);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_ORDER_REASON  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [REASON_NAME_TH]=@Order_R_Decription, [REASON_NAME_EN]=@Order_R_Decription, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [REASON_CODE]=@Order_Reason ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Order_R_Decription", SqlDbType.NVarChar);
                                    command.Parameters["@Order_R_Decription"].Value = data.Order_R_Decription;
                                    command.Parameters.Add("@Order_Reason", SqlDbType.NVarChar);
                                    command.Parameters["@Order_Reason"].Value = data.Order_Reason;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_ORDER_REASON ([REASON_CODE], [REASON_NAME_TH], [REASON_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Order_Reason, @Order_R_Decription, @Order_R_Decription, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Order_Reason", SqlDbType.NVarChar);
                                        command.Parameters["@Order_Reason"].Value = data.Order_Reason;
                                        command.Parameters.Add("@Order_R_Decription", SqlDbType.NVarChar);
                                        command.Parameters["@Order_R_Decription"].Value = data.Order_R_Decription;
