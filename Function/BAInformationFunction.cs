...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundBAInformationRequest;
using AzureFunctionApp.Model.Response.OutboundBAInformationResponse;
...
{
    public  class BAInformationFunction
    {
        ...
        public BAInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("BAInformationFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. BAInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI025', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundBAInformationRequest reqData = new OutboundBAInformationRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI025";
                input.Table_Object = "V_TGSB";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation);
                ...
                OutboundBAInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundBAInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundBAInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundBAInformation);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE ORG_BUSINESS_AREA  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [BUSS_AREA_NAME_TH]=@Business_Area_Desc, [BUSS_AREA_NAME_EN]=@Business_Area_Desc, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [BUSS_AREA_CODE]=@Business_Area ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Business_Area_Desc", SqlDbType.NVarChar);
                                    command.Parameters["@Business_Area_Desc"].Value = data.Business_Area_Desc;
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO ORG_BUSINESS_AREA ([BUSS_AREA_CODE], [BUSS_AREA_NAME_TH], [BUSS_AREA_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Business_Area, @Business_Area_Desc, @Business_Area_Desc, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Business_Area", SqlDbType.NVarChar);
                                        command.Parameters["@Business_Area"].Value = data.Business_Area;
                                        command.Parameters.Add("@Business_Area_Desc", SqlDbType.NVarChar);
                                        command.Parameters["@Business_Area_Desc"].Value = data.Business_Area_Desc;
