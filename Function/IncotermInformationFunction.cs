...
using AzureFunctionApp.Model.Response.OutboundIncotermInformationResponse;
...
{
    public  class IncotermInformationFunction
    {
        ...
        public IncotermInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("IncotermInformationFunction")]
        public  async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. IncotermInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI028', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            using (var client = new HttpClient())
            {
                ...
                input.Interface_ID = "ZSOMI028";
                input.Table_Object = "V_TINC";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation);
                ...
                OutboundIncotermInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundIncotermInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundIncotermInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundIncotermInformation);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_ORDER_INCOTERM  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [DESCRIPTION]=@Description, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [INCOTERM_CODE]=@Incoterms ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                    command.Parameters["@Description"].Value = data.Description;
                                    command.Parameters.Add("@Incoterms", SqlDbType.NVarChar);
                                    command.Parameters["@Incoterms"].Value = data.Incoterms;
                                    QueryUtils.configParameter(command);
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_ORDER_INCOTERM ([INCOTERM_CODE], [DESCRIPTION], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Incoterms, @Description, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Incoterms", SqlDbType.NVarChar);
                                        command.Parameters["@Incoterms"].Value = data.Incoterms;
                                        command.Parameters.Add("@Description", SqlDbType.NVarChar);
                                        command.Parameters["@Description"].Value = data.Description;
