...
using AzureFunctionApp.Common;
using AzureFunctionApp.Model.Request.OutboundCountryInformationRequest;
using AzureFunctionApp.Model.Response.OutboundCountryInformationResponse;
...
{
    public class CountryInformationFunction
    {
        ...
        public CountryInformationFunction(IConfiguration configuration)
        {
        ...
        [Function("CountryInformationFunction")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Anonymous, "get", "post")] HttpRequestData req,
            FunctionContext executionContext)
        {
            var logger = executionContext.GetLogger(CommonConstant.GetLoggerString);
            logger.LogInformation("C# HTTP trigger function processed a request. CountryInformationFunction");
            ...
            using (SqlConnection connection = new SqlConnection(connectionString_))
            {
                ...
                try
                {
                    ...
                    bulder.AppendFormat(" VALUES(@VAL_SYNC_DATA_LOG_SEQ, 'ZSOMI024', 'SYSTEM', dbo.GET_SYSDATETIME()); ");
            ...
            {
                OutboundCountryInformationRequest reqData = new OutboundCountryInformationRequest();
                RequestInput input = new RequestInput();
                input.Interface_ID = "ZSOMI024";
                input.Table_Object = "V_T005K";
                input.All_data = "X";
                reqData.Input = input;

                client.BaseAddress = new Uri(ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation);
                ...
                string fullPath = ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation;
                var jsonVal = JsonConvert.SerializeObject(reqData);
                var content = new StringContent(jsonVal, Encoding.UTF8, "application/json");
                logger.LogDebug("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation);
                logger.LogDebug("=========== jsonVal ================");
                logger.LogDebug("REQUEST:" + jsonVal);
                logger.LogDebug("REQUEST:" + JObject.Parse(jsonVal));
                Console.WriteLine("Call Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation);
                ...
                OutboundCountryInformationResponse resultOutbound = null;
                // SQL Scope
                using (SqlConnection connection = new SqlConnection(connectionString_))
                {
                    connection.Open();
                    ...
                    try
                    {
                        resultOutbound = new OutboundCountryInformationResponse();
                        List<Data> lst = new List<Data>();

                        if (response.IsSuccessStatusCode) // Call Success
                        {
                            resultOutbound = await response.Content.ReadAsAsync<OutboundCountryInformationResponse>();
                        ...
                        logger.LogDebug("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation);
                        logger.LogDebug("Response:" + JsonConvert.SerializeObject(resultOutbound));
                        Console.WriteLine("Response Outbound Service:" + ic.InterfaceSapUrl + CommonConstant.API_OutboundCountryInformation);
                        ...
                        using (SqlConnection connection = new SqlConnection(connectionString_))
                        {
                            ...
                            if (resultOutbound.Data != null && resultOutbound.Data.Count != 0)
                            {
                                ...
                                foreach (Data data in resultOutbound.Data)
                                {
                                    ...
                                    bulder.AppendFormat(" UPDATE MS_COUNTRY  ");
                                    bulder.AppendFormat(" SET [SYNC_ID]=@VAL_SYNC_DATA_LOG_SEQ, [LANG_KEY]=@Lang_Key, [COUNTRY_NAME_TH]=@Name, [COUNTRY_NAME_EN]=@Name, [ACTIVE_FLAG]='Y', [UPDATE_USER]='SYSTEM', [UPDATE_DTM]=dbo.GET_SYSDATETIME() ");
                                    bulder.AppendFormat(" WHERE [COUNTRY_CODE]=@Country_Key ");
                                    command.Parameters.Add("@VAL_SYNC_DATA_LOG_SEQ", SqlDbType.Decimal);
                                    command.Parameters["@VAL_SYNC_DATA_LOG_SEQ"].Value = VAL_SYNC_DATA_LOG_SEQ;
                                    command.Parameters.Add("@Lang_Key", SqlDbType.NVarChar);
                                    command.Parameters["@Lang_Key"].Value = data.Lang_Key;
                                    ...
                                    if (rowsAffected == 0)
                                    {
                                        ...
                                        command.Parameters.Clear();
                                        bulder = new StringBuilder();
                                        bulder.AppendFormat(" INSERT INTO MS_COUNTRY ([COUNTRY_CODE], [LANG_KEY], [COUNTRY_NAME_TH], [COUNTRY_NAME_EN], [ACTIVE_FLAG], [SYNC_ID], [CREATE_USER], [CREATE_DTM], [UPDATE_USER], [UPDATE_DTM])  ");
                                        bulder.AppendFormat(" VALUES(@Country_Key, @Lang_Key, @Name, @Name, 'Y', @VAL_SYNC_DATA_LOG_SEQ, 'SYSTEM', dbo.GET_SYSDATETIME(), 'SYSTEM', dbo.GET_SYSDATETIME()) ");
                                        command.Parameters.Add("@Country_Key", SqlDbType.NVarChar);
                                        command.Parameters["@Country_Key"].Value = data.Country_Key;
                                        command.Parameters.Add("@Lang_Key", SqlDbType.NVarChar);
                                        command.Parameters["@Lang_Key"].Value = data.Lang_Key;
